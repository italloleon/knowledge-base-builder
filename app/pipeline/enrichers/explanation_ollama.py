"""Ollama-based explanation enricher — fallback for gabarito comentado generation.

Uses the local Ollama instance (Gemma or similar) to produce structured explanations
for nursing exam questions. Mirrors explanation_gemini.py but targets the Ollama
/api/chat endpoint.
...
async def generate_explanation(
    ...
    """Generate a gabarito comentado for a single question using Ollama.
    ...
    try:
        ...
        logger.warning("Q%d: Ollama explanation request timed out", question.number)
        return None
    except httpx.HTTPError as exc:
        logger.warning("Q%d: Ollama HTTP error during explanation: %s", question.number, exc)
        return None
    except Exception as exc:  # noqa: BLE001
        logger.warning("Q%d: unexpected Ollama explanation error: %s", question.number, exc)
        return None

    parsed = _extract_json(raw_content)
    if parsed is None:
        logger.warning(
            "Q%d: failed to parse JSON from Ollama explanation response", question.number
        )
        return None
...
async def generate_explanations(
    ...
    for idx, q in enumerate(enrichable, start=1):
        logger.info(
            "Stage 5 (Ollama): generating explanation Q%d (%d/%d)",
            q.number,
            idx,
            total,
        )
        ...
        try:
            result = await generate_explanation(q, gabarito, enrichment)
        except Exception as exc:  # noqa: BLE001
